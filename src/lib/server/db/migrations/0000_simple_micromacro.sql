CREATE TYPE "public"."entry_status" AS ENUM('draft', 'pending', 'published', 'rejected', 'deleted', 'archived');--> statement-breakpoint
CREATE TYPE "public"."license" AS ENUM('cc-by', 'cc-by-sa', 'cc-by-nc', 'cc-by-nc-sa', 'cc-by-nd', 'cc-by-nc-nd', 'cc0');--> statement-breakpoint
CREATE TYPE "public"."media_type" AS ENUM('image', 'video', 'audio', 'archive');--> statement-breakpoint
...
CREATE TABLE "reaction" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "reaction_user_id_entry_id_pk" PRIMARY KEY("user_id","entry_id")
);
--> statement-breakpoint
CREATE TABLE "rule" (
	"id" serial PRIMARY KEY NOT NULL,
...
CREATE TABLE "vote" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "vote_entry_id_user_id_pk" PRIMARY KEY("entry_id","user_id")
);
--> statement-breakpoint
...
CREATE INDEX "entrys_search_idx" ON "entry" USING gin (to_tsvector('english', "title"));--> statement-breakpoint
