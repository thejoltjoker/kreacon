...
export default class ConfigProvider {
	...
	public getSpekeArn(): string {
		return this.getOrThrowException(
			'SPEKE_ARN',
			'The role ARN allowing access to the SPEKE server'
		);
	}
	...
	public getSpekeGatewayRegion(): string {
		return this.getOrThrowException(
			...
			'The region of the associated AWS Gateway'
		);
	}
	...
	private getOrThrowException(key: string, description: string): any {
		...
		console.error(
			'\x1b[31m',
			`[MissingArgument] '${key}' - '${description}' could not be found in the config source.`
		);
		process.exit(-1);
	}
