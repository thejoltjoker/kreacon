<script lang="ts" module>
	...
	export const inputVariants = tv({
		...
		variants: {
			variant: {
				default:
					'focus:border-violet-500 border-shade-600 bg-bg focus:ring-violet-500 focus:placeholder:text-shade-600 [&:read-only]:border-shade-400 [&:read-only]:bg-muted-background/50 [&:read-only]:text-shade-400 disabled:border-shade-400 disabled:bg-muted-background/50 disabled:text-shade-400 [&:user-invalid]:border-red-500 [&:user-invalid]:focus:border-red-500 [&:user-invalid]:focus:ring-red-500',
				ghost:
