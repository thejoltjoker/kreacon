...
<script lang="ts">
	...
	import { ArrowUpDownIcon, GalleryThumbnailsIcon, ImageIcon, Settings2Icon } from 'lucide-svelte';
	interface Item {
		[key: string]: any;
		id: number | string;
		thumbnailUrl?: string;
	...
	>
		...
		{#each fieldsToRender as [key]}
			<li class="flex flex-1 items-center">
				<button class="flex items-center gap-sm overflow-hidden font-bold">
