...
const logger = createBackendLogger('api/uploads/thumbnail');
...
export const PUT: RequestHandler = async ({ request, locals, url }) => {
	...
	if (!isEmailVerified(locals)) {
		logger.warn(`Unverified user ${locals.user?.id} attempted to upload thumbnail`);
	...
	try {
		...
		logger.error('Failed to process thumbnail', errorObj);

		const errorMessage = errorObj.message;
		return error(StatusCodes.INTERNAL_SERVER_ERROR, {
			message: `Failed to process thumbnail: ${errorMessage}`
		});
	}
};
